%% @doc update_store_address_handler

-module(update_store_address_handler).
...
init(Req0, Opts) ->
	...
	io:format("~n in update store address handler firebase uid ~n ~p~n", [FireBaseUid]),
    ...
    #{num_rows := N} = 
	db_helpers:update_store_address(
